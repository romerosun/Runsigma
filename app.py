...
ASSET_PATH = Path(__file__).parent / "assets" / "UAL1_Standard.glb"
...
<style>
  :root {{
    --bg:#050914; --panel:#0a1428; --panel2:#0d1b35; --line:#1e3e70;
  ...
  * {{ box-sizing:border-box; }}
  ...
  .wrap {{ padding:26px; }}
  .hero {{
    border:1px solid var(--line); background:linear-gradient(135deg,#0c1b36,#081225 70%);
    border-radius:22px; padding:26px; box-shadow:0 20px 70px rgba(0,0,0,.35);
  }}
  ...
  button {{
    border:1px solid #3989ff; background:#092b62; color:white; padding:13px 18px;
    border-radius:12px; font-weight:800; cursor:pointer; box-shadow:inset 0 0 0 1px rgba(255,255,255,.05);
  }}
  ...
  .badge {{ position:absolute; left:24px; top:24px; z-index:2; border:1px solid #3989ff; color:white; background:rgba(9,43,98,.75); padding:12px 16px; border-radius:14px; font-weight:900; }}
  ...
  .row {{ display:grid; grid-template-columns:42px 1fr 92px; gap:14px; align-items:center; padding:15px 8px; border-bottom:1px solid rgba(80,130,200,.16); opacity:.35; transition:.3s; }}
  ...
  .sched {{ position:relative; margin-top:10px; border:1px solid #203a61; border-radius:16px; background:#040b18; padding:16px; }}
  ...
  .timeline {{ position:relative; height:36px; background:repeating-linear-gradient(90deg,rgba(120,160,220,.11) 0, rgba(120,160,220,.11) 1px, transparent 1px, transparent 8.333%); border-radius:10px; overflow:hidden; }}
  .block {{ position:absolute; top:6px; height:24px; border-radius:7px; background:linear-gradient(90deg,#55a7ff,#76ddff); box-shadow:0 0 18px rgba(85,167,255,.35); }}
  ...
  <div class="hero">
    <div class="top">
      <div>
        ...
        <div class="sub">The algorithm builds labor hours from the actual work motions, then converts the standard time into staffing blocks that can be scheduled from 6 AM to 6 PM.</div>
      </div>
      <div class="btns">
...
const motions = [
  {{name:'Approach', desc:'Worker positions body toward the item', tmu:10, pct:16, cue:.05}},
  {{name:'Reach', desc:'Arm extends toward the object', tmu:16, pct:32, cue:.20}},
  {{name:'Bend', desc:'Body lowers to access the item', tmu:30, pct:52, cue:.38}},
  {{name:'Grasp', desc:'Hand gains control of the item', tmu:3, pct:66, cue:.55}},
  {{name:'Lift', desc:'Object is raised under control', tmu:16, pct:84, cue:.74}},
  {{name:'Return', desc:'Worker returns to stable posture', tmu:10, pct:100, cue:.90}}
];
...
const grid = new THREE.GridHelper(8, 28, 0x1e5aa0, 0x183250); grid.position.y=0; scene.add(grid);
...
function buildMostLine() {{
  ...
  for(let i=0;i<step;i++) {{
    ...
    line.innerHTML += `<div class="stack" style="left:${{left}}%;top:${{7+i*4}}px;width:${{width}}%"></div>`;
  }}
}}
renderRows();
...
new GLTFLoader().load(url, (gltf)=>{{
  ...
  if(gltf.animations && gltf.animations.length) {{
    ...
    document.getElementById('status').textContent = 'Loaded: UAL1_Standard.glb | ' + gltf.animations.length + ' animations';
  }} else {{
    document.getElementById('status').textContent = 'Loaded model | no embedded animation';
  }}
}}, undefined, (err)=>{{ document.getElementById('status').textContent='Could not load GLB'; console.error(err); }});

window.playCurrent = function() {{ if(activeAction) activeAction.paused=false; }}
...
function animate() {{
  ...
  if(mixer) {{ mixer.update(dt); if(activeAction && !activeAction.paused) {{
